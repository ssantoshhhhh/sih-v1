...
export async function updateSession(request: NextRequest) {
  ...
  if (!supabaseUrl || !supabaseAnonKey || supabaseUrl.trim() === "" || supabaseAnonKey.trim() === "") {
    console.error("[v0] Missing or empty Supabase environment variables, skipping auth check")
  ...
  try {
    ...
    console.error("[v0] Continuing without authentication")
    // Return response without authentication checks
    return supabaseResponse
  }
